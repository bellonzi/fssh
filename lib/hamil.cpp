...
/* * * * * * * * * * * * * * * * * * * * * * * */
void hamil::HamilD(const params &config, traj &curr_traj, rk4 &k) {
  // updating Hamil if using Runge Kutta 4

  // Hamiltonian with flat adiabats
  double theta = two_pi * (jlx * std::erf(jbx * (curr_traj.x(0) + k.x(0))) + 1.0) / 4.0;
  double dthetadx0 =
      two_pi * jlx * jbx * std::exp(std::pow(jbx * (curr_traj.x(0) + k.x(0)), 2)) / 4.0;
  double phi = 0.3 * jlx * (curr_traj.x(1) + k.x(1));
  double dphidx1 = 0.3 * jlx;
...
/* * * * * * * * * * * * * * * * * * * * * * * */
void hamil::HamilA(const params &config, traj &curr_traj, rk4 &k) {

  // update Ve
  HamilD(config, curr_traj, k);
