...
/* * * * * * * * * * * * * * * * * * * * * * * */
  void HamilD(){
    ...
    double dthetadx0 = two_pi*jlx*jbx*std::exp(std::pow(jbx*traj::x(0),2))/4.0;
    ...
    Fe(0,1,0) = jA*std::cos(theta)*std::exp(arma::cx_double(0,phi))*dthetadx0;
    Fe(1,0,0) = conj(Fe(0,1,0));
          
    Fe(0,1,1) = jA*std::sin(theta)*std::exp(arma::cx_double(0,phi))*arma::cx_double(0,dphidx1);
